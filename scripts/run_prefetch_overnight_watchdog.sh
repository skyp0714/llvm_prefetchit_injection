...
start_main_session() {
  ...
  local cmd
  cmd="cd ${REPO_ROOT} && TRACE_INPUTS='${TRACE_INPUTS}' RUN_ID='${RUN_ID}' MIN_IMPROVEMENT_ABS_PCT='${MIN_IMPROVEMENT_ABS_PCT}' PATIENCE_BATCHES='${PATIENCE_BATCHES}' SCREEN_ITERATIONS='${SCREEN_ITERATIONS}' FINAL_ITERATIONS='${FINAL_ITERATIONS}' RUN_RESIDUAL_TRACE='${RUN_RESIDUAL_TRACE}' RUN_FINAL_COMPARE='${RUN_FINAL_COMPARE}' MAX_BATCHES='${MAX_BATCHES}' BUILD_JOBS='${BUILD_JOBS}' ${LLVM_PREFETCH_DIR}/scripts/run_prefetch_until_plateau.sh"
