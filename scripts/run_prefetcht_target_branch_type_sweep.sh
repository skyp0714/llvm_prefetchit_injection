...
RUN_ID="${RUN_ID:-prefetcht_target_branch_types_$(date +%Y%m%d_%H%M%S)}"
OUT_DIR="${OUT_DIR:-${LLVM_PREFETCH_DIR}/results/prefetcht_target_branch_type_sweep/${RUN_ID}}"
RUNS_DIR="${OUT_DIR}/runs"
LOG_DIR="${OUT_DIR}/logs"
PLOTS_DIR="${OUT_DIR}/plots"
LOG="${OUT_DIR}/target_branch_type_sweep.log"
...
SITE_BRANCH_TYPE_FILTER="${SITE_BRANCH_TYPE_FILTER:-}"
...
write_variant_csv() {
  ...
  for branch in ${BRANCH_TYPES}; do
    idx=$((idx + 1))
    variant="v$(printf '%02d' "${idx}")_target_branch_$(lower_branch "${branch}")"
    plot_label="${branch}"
...
build_one() {
  ...
  log "build start ${variant}: target_branch=${branch} coverage=${TARGET_COVERAGE_PCT}% d=${DEPTH_MIN}-${DEPTH} budget=${SITE_BUDGET} offsets=${PREFETCH_BYTE_OFFSETS} site_filter=${SITE_BRANCH_TYPE_FILTER:-all}"
  set +e
  ...
  BRANCH_DEPTH_POLICY="${BRANCH_DEPTH_POLICY}" \
  BRANCH_TYPE_FILTER="${SITE_BRANCH_TYPE_FILTER}" \
  SAMPLE_BRANCH_TYPE_FILTER="${branch}" \
...
target_branch_types=${BRANCH_TYPES}
site_branch_type_filter=${SITE_BRANCH_TYPE_FILTER:-all}
best_plan=${BEST_PLAN}
trace_inputs=${TRACE_INPUTS}
baseline_binary=${BASELINE_BINARY}
source_work=${SOURCE_WORK}
...
plot_results() {
  ...
  python3 "${LLVM_PREFETCH_DIR}/tools/plot_branch_type_prefetch_results.py" \
    ...
    --title "Prefetcht1 Target Branch-Type PGO" \
    --xlabel "Target Branch Type" \
...
write_manifest
log "target-branch-type prefetcht1 sweep start: ${OUT_DIR}"
log "target_branches=${BRANCH_TYPES}"
log "site_branch_filter=${SITE_BRANCH_TYPE_FILTER:-all}"
...
profile_successful_builds
plot_results
log "target-branch-type prefetcht1 sweep complete"
